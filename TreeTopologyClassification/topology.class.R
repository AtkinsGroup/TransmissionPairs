topology.class <-
    function(tr,
        ...
        if (missing(outgroup))
            outgroup = TRUE
        if (missing(p.ancestral.logs))
            p.ancestral.logs = TRUE
        ...
        for (i in 1:length(tr)) {
                    ...
                    for (child in Children) {
                        ...
                        if (child %in% tips) {
                            ...
                            if (length(unique(taxtips)) == 1) {
                                ...
                                nodes.eval <-
                                    nodes.eval[!nodes.eval %in% c(child, numnodes)] #remove nodes (they are monophyletic) from nodes.eval
                                if ((unique(taxtips)) == states.recipient) {
                                    lineages.recipient <-
                                        lineages.recipient + 1
                                } else {
                                    lineages.source <- lineages.source + 1
            ...
            ####################################
            #Tree calculations--directionality
            ####################################
            states.pars <-
            ...
            states.ml.ER <-
                setNames(factor(states$state), factor(states$accession)) %>% ace(.,
                                                                                 troot,
                                                                                 method = "ML",
                                                                                 model = "ER",
                                                                                 type = "discrete")
            ...
            vector.lineages.recipient <-
                c(vector.lineages.recipient, lineages.recipient)
            ...
            if (p.ancestral.logs) {
                ...
                vector.p.ancestral.bayes <-
                    c(vector.p.ancestral.bayes, p.ancestral.bayes)
                vector.bayes.direction <-
                    c(vector.bayes.direction, bayes.direction)
            }
            vector.tree.length <-
                c(vector.tree.length, max(distRoot(troot)))
            vector.tree.SBL <-
                c(vector.tree.SBL, sum(troot$edge.length))
            vector.tree.SPD <- c(vector.tree.SPD, sum(distTips(troot)))
            vector.tree.MBL <-
                c(vector.tree.MBL, mean(troot$edge.length))
            vector.tree.MPD <-
                c(vector.tree.MPD, mean(distTips(troot)))
            vector.tree.VBL <-
                c(vector.tree.VBL, var(troot$edge.length))
        ...
        return(
            list(
                ...
                'vector.tree.length' = vector.tree.length,
                'vector.tree.SBL' = vector.tree.SBL,
                'vector.tree.SPD' = vector.tree.SPD,
                'vector.tree.MBL' = vector.tree.MBL,
                'vector.tree.MPD' = vector.tree.MPD,
                'vector.tree.VBL' = vector.tree.VBL,
                'vector.tree.VPD' = vector.tree.VPD,
                'logs' = logs
            )
        )
