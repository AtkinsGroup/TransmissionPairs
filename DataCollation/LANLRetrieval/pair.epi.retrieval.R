pair.epi.retrieval<-function(query, key){
    ...
    ####################################
    query<-read.csv(query, header = FALSE, 
    ...
    for(id in 1:nrow(query)){ 
        ...
        print(paste("Queuing the Los Alamos db for cluster ",
                    cluster,", source patient ",
                    source," & recipient patient ",
                    recipient,sep=""))
        ####################################
        ...
        ####################################
        info<-read.delim(paste("cluster_",
                                 cluster,"_clinical.tsv",sep=""), 
                           header = FALSE, stringsAsFactors =FALSE) 
        info.gb<-read.delim(paste("cluster_",
        ...
        source.row<-as.numeric(
            rownames(info)[grepl(source,info[,source.col])]) 
        recipient.col<-names(info[,1:2])[grepl(recipient,info[,1:2])] 
        recipient.row<-as.numeric(
            rownames(info)[grepl(recipient,info[,recipient.col])]) 
        info$States<-"NA" 
        ...
        ####################################        
        if(nrow(info)>2){
            cluster.name<-paste(as.character((unique(info$ClusterName))),".",noquote(info[recipient.row,"PatientCode"]),sep="")
        }else{
        ...
        write.csv(info, paste(cluster.name, "/info.csv", sep=""), 
                  quote=T, row.names = F)
        write.csv(info.gb, paste(cluster.name, "/info.gb.csv", sep=""), 
                  quote=T, row.names = F)
        system(paste("rm cluster_", cluster, "_clinical.tsv ", "cluster_",
                     cluster,"_accessions.tsv ",  sep="")) 
    }
    return(ids)
}
