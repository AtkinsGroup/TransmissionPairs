pair.sequence.retrieval<-function(ids, min.overlap, max.length.diff, min.seqs){
    ...
    for(id in ids){     
        ...
        info.gb<-read.csv(paste(cluster.name, "/", "info.gb.csv", sep=""), 
                          stringsAsFactors = F) 
        info.gb<-info.gb[,c("row_id", "patient_id", "accession_id", "seq_length",
                            "pos","pos_ini","pos_end", "subtype")]
        ...
        ####################################
        set.id<-data.frame( "cluster_name"=character(), "cluster_id"=integer(), 
                              "donor.id"=integer(), "recipient.id"=integer(),
                              "subtype"=character(), "ref" = character(), 
                              "counts" = integer(), "source.n"=integer(), 
                              "recipient.n"=integer(), "min" = integer(), 
                              "max" = integer(), "length" = integer(), 
                              "accessions" = character(), stringsAsFactors = FALSE) 
        ####################################
        #define datasets based on sequence length
        ####################################
        while(nrow(info.gb) >= min.seqs){ 
            newSet<-character(); toRemove<-character() 
            accession.min<-numeric(); accession.max<-numeric() 
            minSeq_length<-as.integer(min(info.gb$seq_length)) 
            ref<-as.character(info.gb$accession_id[which(
                info.gb$seq_length == minSeq_length)])[1] 
            ...
            #validate the set (Minimum number of sequences criteria) 
